...
public class Uploader
{
    ...
    public string UploadImage(HttpPostedFileBase fileToUpload, string outputPath, int newWidth, bool randomFileName, bool createThumbnail, int thumbnailWidth = 100)
    {
        string strFileName;
        string strLongFilePath = fileToUpload.FileName;
        ...
        if (imgFor != null)
        {
            ...
            string filePath = HttpContext.Current.Request.PhysicalApplicationPath + outputPath;
            fileToUpload.SaveAs(filePath + strFileName);

            if (newWidth > 0)
            {
                this.ReSizeImage(filePath + strFileName, filePath, newWidth);
            }

            if (createThumbnail)
            {
                CreateThumbnail(fileToUpload, outputPath, strFileName, thumbnailWidth);
            }
            return strFileName;
        }
        else
    ...
    public void CreateThumbnail(HttpPostedFileBase fileToUpload, string outputPath, string fileName, int width)
    {
        string filePath = HttpContext.Current.Request.PhysicalApplicationPath + outputPath;
        fileToUpload.SaveAs(filePath + "tn_" + fileName);

        if (width > 0)
        {
            this.ReSizeImage(filePath + "tn_" + fileName, filePath, width);
        }
    }
